...
public class UserManagementService {
	...
	public LoginResponse userLogin(LoginRequest loginRequest) throws Exception{
		...
		LoginResponse loginResponse = new LoginResponse();
			...
			//User userEntity = userRepository.findByEmailAndPasswordAndUserType(loginRequest.getEmail(), loginRequest.getPassword(),"USER");
			User userEntity = userRepository.findByEmailAndPasswordAndUserTypeAndIsDelete(loginRequest.getEmail(), loginRequest.getPassword(),"USER","N");
			if(userEntity==null) {
				throw new ServiceException(AppConstant.USER_ERROR_CODE1,AppConstant.errormap.get(AppConstant.USER_ERROR_CODE1));
			}
			else if(userEntity.getIsActive().equals("N"))
			{
				throw new ServiceException(AppConstant.USER_ERROR_CODE2,AppConstant.errormap.get(AppConstant.USER_ERROR_CODE2));
			}
			else if(userEntity.getIsDelete().equals("Y"))
			{
				throw new ServiceException(AppConstant.USER_ERROR_CODE3,AppConstant.errormap.get(AppConstant.USER_ERROR_CODE3));
			}			
			else
			{
			...
			if(subsList.isEmpty() || subsList.get(0).getAccountExpireDate().getTime() < new Date().getTime()) {
				throw new ServiceException(AppConstant.USER_ERROR_CODE4,AppConstant.errormap.get(AppConstant.USER_ERROR_CODE4));
			}
			...
			if(lastLoginTime !=null && ((lastLoginTime.getDate() != currentDate.getDate()) || 
			    ...
			    headers.add("Authorization", "Basic YXBpLWV4aW13YXRjaDp1ZTg0Q1JSZnRAWGhBMyRG");
				 ...
				 ResponseEntity<Subscription> responseEntity = restTemplate.exchange(subscriptionUrl + userSubscription.getSubscriptionId(),
	...
	public ForgotPasswordResponse forgotPassword(String userEmail) throws Exception{
		...
		if(userEntity==null) {
			throw new ServiceException(AppConstant.USER_ERROR_CODE5,AppConstant.errormap.get(AppConstant.USER_ERROR_CODE5));
		}
		else if(userEntity.getIsActive().equals("N"))
		{
			throw new ServiceException(AppConstant.USER_ERROR_CODE2,AppConstant.errormap.get(AppConstant.USER_ERROR_CODE2));
	...
	private boolean sendEmail(String emailId) {
		try {
		    ...
		    message.addRecipient(Message.RecipientType.TO, new InternetAddress(emailId)); //Send email To (Type email ID that you want to send)
	...
	private boolean forgotPasswordEmail(String emailId, String firstName, String randomNumber) {
		try {
		    ...
		    msgBody = "Dear "+firstName+",<br><br>We have received a request to reset your password for the BE Datos Online Platform. If you did not initiate this request, please kindly disregard this email.<br><br>";
		    msgBody = msgBody + "Here is your 4 digit auto generated pin: "+randomNumber+".<br><br>";
		    msgBody = msgBody + "Please be advised that this pin will expire after a certain period for security reasons.";
		    msgBody = msgBody + "Should you have any questions or concerns, please do not hesitate to contact our support team at (+91) 9073371811/9051062400.<br><br>";
	...
	public LoginResponse adminLogin(LoginRequest loginRequest) throws Exception{
		...
		LoginResponse loginResponse = new LoginResponse();
			...
			User userEntity = userRepository.findByEmailAndPasswordAndUserType(loginRequest.getEmail(), loginRequest.getPassword(),"ADMIN");
			if(userEntity==null) {
				throw new ServiceException(AppConstant.USER_ERROR_CODE1,AppConstant.errormap.get(AppConstant.USER_ERROR_CODE1));
			}
			else if(userEntity.getIsActive().equals("N"))
	...
	public void userCreate(UserRequest request, Long accessedBy) throws Exception{		
				...
				if(user!=null) {
					...
					if(null!=firstname) userEntity.setFirstname(firstname);
					if(null!=lastname) userEntity.setLastname(lastname);
					if(null!=email) userEntity.setEmail(email);
					if(null!=mobile) userEntity.setMobile(mobile);
					if(null!=companyName) userEntity.setCompanyName(companyName);
					if(null!=userType) userEntity.setUserType(userType);					
					if(null!=isActive) userEntity.setIsActive(request.getIsActive()); else userEntity.setIsActive("N");
					userEntity.setIsDelete("N");
					if(null!=password) userEntity.setPassword(request.getPassword()); else userEntity.setPassword("Be12345#");
					userEntity.setCreatedDate(new Date());
					if(null!=uplineId) userEntity.setUplineId(uplineId); else userEntity.setUplineId((long) 0);
					if(null!=downloadLimit) userEntity.setDownloadLimit(downloadLimit); else userEntity.setDownloadLimit((long) 0);
					if(null!=accessedBy) userEntity.setCreatedBy(accessedBy);
					if(null!=memberId) userEntity.setMemberId(memberId);
	...
	public void userUpdate(UserRequest request,Long userId, Long accessedBy) throws Exception{
				...
				if(userEntity==null ) {
					...
					if(null!=request.getFirstname()) userEntity.setFirstname(request.getFirstname());
					if(null!=request.getLastname()) userEntity.setLastname(request.getLastname());
					if(null!=request.getCompanyName()) userEntity.setCompanyName(request.getCompanyName());
					if(null!=request.getUserType()) userEntity.setUserType(request.getUserType());
					if(null!=request.getPassword()) userEntity.setPassword(request.getPassword());
					if(null!=request.getMobile()) userEntity.setMobile(request.getMobile());
					if(null!=request.getIsActive()) userEntity.setIsActive(request.getIsActive());
					if(null!=request.getIsDelete()) userEntity.setIsDelete(request.getIsDelete());					
					if(null!=request.getDownloadLimit()) userEntity.setDownloadLimit(request.getDownloadLimit());
					if(null!=request.getMemberId()) userEntity.setMemberId(request.getMemberId());
	...
	public void userDelete(UserRequest request,Long userId, Long accessedBy) throws Exception{
		...
		if(userEntity==null ) {
			...
			if(userEntity !=null) {
				...
				List<UserSubscription> srcList = userSubscriptionRepository.findAllByUserIdAndIsActiveOrderByIdDesc(accessedBy,"Y");
	...
	public UserListResponse userList(Long uplineId, String userType, Long accessedBy, String isExpired, String isActive) throws Exception{
		...
		if(null!=srcList && !srcList.isEmpty()) {
			...
			for(User user:srcList) {
				...
				List<UserSubscription> srcList1 = userSubscriptionRepository.findAllByUserIdAndIsActiveOrderByIdDesc(user.getId(),"Y");
				if(null!=srcList1) {
	...
	public void createUserSubscription(@Valid UserSubscriptionRequest userSubscriptionRequest, Long accessedBy) {
		...
		List<UserSubscription> srcList = userSubscriptionRepository.findAllByUserIdOrderByIdDesc(userSubscriptionRequest.getUserId());
		 ...
		 * Subscription.class);
		 */
	    ResponseEntity<Subscription> responseEntity = restTemplate.exchange(subscriptionUrl + userSubscriptionRequest.getSubscriptionId(),
	    		HttpMethod.GET, new HttpEntity<Object>(headers),
	    		Subscription.class);
		Subscription subscription = responseEntity.getBody();
		...
		if(null != userSubscriptionRequest.getIsActive() ) userSubscriptionEntity.setIsActive(userSubscriptionRequest.getIsActive()); else userSubscriptionEntity.setIsActive("Y");
	...
	private boolean userWelcomeEmail(Long userId, String subscriptionName, String accountExpireDate) {
		try {
		    ...
		    msgBody = "Dear "+userEntity.getFirstname()+",<br><br>Thank you for choosing BE Datos online Platform and  Find below activation Details to help you get started.<br><br>";
		    msgBody = msgBody + "As This platform will help you to approach your business objectives with greater intelligence.<br><br>";
		    ...
		    msgBody = msgBody + "If you have any questions or need assistance, feel free to reach out to our customer support team at (+91) 9073371811/9051062400.<br><br>";
		    msgBody = msgBody + "We're excited to embark on this journey with warm welcome and look forward to seeing you thrive on our web based Portal.<br><br><br>";
		    msgBody = msgBody + "Best regards,<br><br>";
		    ...
		    Address[] from = InternetAddress.parse("accounts@eximwatch.com");//Your domain email
		    message.addFrom(from);
		    message.addRecipient(Message.RecipientType.TO, new InternetAddress(userEntity.getEmail())); //Send email To (Type email ID that you want to send)
	...
	private boolean subUserWelcomeEmail(Long userId, Long uplineid) {
		try {
			...
			List<UserSubscription> srcList = userSubscriptionRepository.findAllByUserIdAndIsActiveOrderByIdDesc(uplineid,"Y");
		    ...
		    msgBody = "Dear "+userEntity.getFirstname()+",<br><br>Your Sub User Login ID details Below mentioned Here.<br><br>";		    
			...
			if(null!=srcList) {
				...
				for(UserSubscription userSubscription:srcList) {					
				    ...
				    msgBody = msgBody + "<b>Subscription Expire Date:</b> "+userSubscription.getAccountExpireDate()+"<br>";
				}
			}
...
public LoginListResponse loginList(Long userId,Long uplineId,int pageNumber, Long accessedBy, Date fromDate, Date toDate) throws Exception{
	...
	public UserSubscriptionList activeUserSubscriptionList(Long userId, Long accessedBy) {
		...
		List<UserSubscription> srcList = userSubscriptionRepository.findAllByUserIdAndIsActiveOrderByIdDesc(userId,"Y");
	...
	public void updateUserSubscription(UserSubscriptionDetailsRequest userSubscriptionDetailsRequest,
			...
			if(userSubscriptionEntity==null ) {
				...
				if(null!=userSubscriptionDetailsRequest.getDownloadLimit()) userSubscriptionEntity.setDownloadLimit(userSubscriptionDetailsRequest.getDownloadLimit());
				if(null!=userSubscriptionDetailsRequest.getTotalWorkspace()) userSubscriptionEntity.setTotalWorkspace(userSubscriptionDetailsRequest.getTotalWorkspace());
				if(null!=userSubscriptionDetailsRequest.getSubUserCount()) userSubscriptionEntity.setSubUserCount(userSubscriptionDetailsRequest.getSubUserCount());
				if(null!=userSubscriptionDetailsRequest.getDataAccessInMonth()) userSubscriptionEntity.setDataAccessInMonth(userSubscriptionDetailsRequest.getDataAccessInMonth());
				if(null!=userSubscriptionDetailsRequest.getDownloadPerDay()) userSubscriptionEntity.setDownloadPerDay(userSubscriptionDetailsRequest.getDownloadPerDay());
				if(null!=userSubscriptionDetailsRequest.getSupport()) userSubscriptionEntity.setSupport(userSubscriptionDetailsRequest.getSupport());
				if(null!=userSubscriptionDetailsRequest.getTicketManager()) userSubscriptionEntity.setTicketManager(userSubscriptionDetailsRequest.getTicketManager());
				if(null!=userSubscriptionDetailsRequest.getRecordsPerWorkspace()) userSubscriptionEntity.setRecordsPerWorkspace(userSubscriptionDetailsRequest.getRecordsPerWorkspace());
				if(null!=userSubscriptionDetailsRequest.getDisplayFields()) userSubscriptionEntity.setDisplayFields(userSubscriptionDetailsRequest.getDisplayFields());
				if(null!=userSubscriptionDetailsRequest.getQueryPerDay()) userSubscriptionEntity.setQueryPerDay(userSubscriptionDetailsRequest.getQueryPerDay());	
				if(null!=userSubscriptionDetailsRequest.getAccountExpireDate()) userSubscriptionEntity.setAccountExpireDate(userSubscriptionDetailsRequest.getAccountExpireDate());
				if(null!=userSubscriptionDetailsRequest.getAllowedChapter()) userSubscriptionEntity.setAllowedChapter(userSubscriptionDetailsRequest.getAllowedChapter());
				if(null!=userSubscriptionDetailsRequest.getDataAccessUpto()) userSubscriptionEntity.setDataAccessUpto(userSubscriptionDetailsRequest.getDataAccessUpto());
				if(null!=userSubscriptionDetailsRequest.getCountryId()) userSubscriptionEntity.setCountries(userSubscriptionDetailsRequest.getCountryId().toString());
